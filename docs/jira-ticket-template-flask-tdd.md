# Jira Ticket Template for Test-First Flask Apps in Ralph Loop

Reusable Jira ticket template that instructs Claude (in the Ralph Loop) to build test-first Flask app skeletons. Contract-level: acceptance criteria + architecture + test scenarios.

---

## Prerequisites (One-Time Setup)

Before running any test tickets, verify these configurations:

### Package Allowlist

File: `.claude/package-allowlist.json` must include these pip packages:

```
flask, flask-sqlalchemy, flask-migrate, python-dotenv
```

### Ralph Config Profile

File: `.claude/ralph-config.json` must have:

```json
"active_profile": "code_repo"
```

This enables `tests_must_pass: true`, `lint_must_pass: true`, and `coverage_threshold: 80`.

### Verify Setup

Run `/preflight` in the agentic-dev-loop directory to confirm the environment is ready.

---

## Reusable Template

Sections marked `[ADJUST]` are swapped out for each variation. Everything else stays the same.
...
1. **Copy** the News Flash example as your starting point
2. **Replace** these sections:
   - Title: Change app name
   - h2. Modellspecifikation: New model name and fields
   - h2. Affarsregler: New service name, rules, and Swedish error messages
   - h2. Routes: New endpoints and behaviors
   - h2. Acceptance Criteria: Adjust the `- [ ]` items to match new model/service/routes
   - h2. Testscenarier: New test tables matching the new business rules
3. **Keep** these sections unchanged:
   - h2. Uppdrag (change only `[APP_NAME]`)
   - h2. Arkitektur (always the same)
   - h2. Projektstruktur (adjust filenames only)

**Key rule for acceptance criteria:** Each `- [ ]` line must be independently verifiable. The sanitize.py extracts these into CURRENT_TASK.md, and they become the agent's exit checklist.
